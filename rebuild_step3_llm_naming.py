...
def run_llm_naming_and_critique():
    ...
    print("=== STEP 3: LLM NAMING + CRITIQUE ===")
    ...
    for i, (community_id, size) in enumerate(community_sizes.items()):
        ...
        # Prepare prompt
        titles_text = "\n".join([f"- {title}" for title in market_titles[:50]])  # Limit to 50 for token efficiency
        if len(market_titles) > 50:
            titles_text += f"\n... and {len(market_titles) - 50} more similar markets"
...
- [problem 3 if applicable]"""
        ...
        if (i + 1) % 10 == 0:
            print(f"✅ Processed {i+1}/{len(community_sizes)} communities")
